...
author: "By Laurent Smeets and Rens van de Schoot, with updates by Duco Veen"
format: 
  html:
    toc: true
...
For the first model with priors we just set normal priors for all regression coefficients, in reality many, many more prior distributions are possible, see the [BRMS manual](https://cran.r-project.org/web/packages/brms/brms.pdf) for an overview.  To place a prior on the fixed intercept, one needs to include `0 + Intercept`. See [here](https://rdrr.io/cran/brms/man/set_prior.html) for an explanation.
```{r}
